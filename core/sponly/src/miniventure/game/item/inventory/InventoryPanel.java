...
public class InventoryPanel extends Table {
	...
	public InventoryPanel(Inventory inv, int minSlots, int maxSlots) { this(inv, minSlots, maxSlots, maxSlots); }
	public InventoryPanel(Inventory inv, int minSlots, int maxSlots, int maxSlotsPerRow) {
		super(VisUI.getSkin());
	...
	class InventorySlot extends ItemSlot {
		...
		InventorySlot(int idx, FetchFunction<Boolean> drawSelected) {
			...
			setBackground(new SlotBackground(
				() -> inv.getSlotsTaken() > idx || minSlots > idx,
				() -> over,
				drawSelected
			));
		...
		@Override @Nullable
		public Item getItem() {
			if(idx < 0 || inv.getSlotsTaken() <= idx)
				return null;
		...
		public int getCount() {
			if(idx < 0 || inv.getSlotsTaken() <= idx)
				return 0;
			return inv.getCount(inv.getItem(idx));
		}
	}
}
