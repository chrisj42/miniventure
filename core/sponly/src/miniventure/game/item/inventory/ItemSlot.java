...
import miniventure.game.item.BreakableItem;
...
public class ItemSlot extends Widget {
	...
	public ItemSlot setItem(@Nullable Item item) {
		...
		if(item == null || !showName)
			...
			prefWidth = ICON_WIDTH + SPACING + GdxCore.getTextLayout(GdxCore.getFont(FontStyle.KeepSize), item.getName()).width;
	...
	@Override public void draw(Batch batch, float parentAlpha) {
		...
		if(item != null) {
			...
			if(item instanceof BreakableItem)
				ItemIcon.renderUsability(batch, getX()+2+XPADDING, getY()+2+YPADDING, ((BreakableItem)item).getDurability());
			
			if(showName || showCount()) {
				BitmapFont font = GdxCore.getFont(FontStyle.KeepSize);
